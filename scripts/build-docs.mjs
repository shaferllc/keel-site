...
// Prefer the sibling checkout when present (keeps the site on tip docs while
// hacking). CI / machines without a sibling fall back to the installed package.
const sources = [
...
// Bake the MCP installer so https://keeljs.com/install.sh stays in sync with the
// framework checkout / package (no raw.githubusercontent dependency for users).
const installCandidates = [
  join(docsDir, "../scripts/install.sh"),
  join(docsDir, "../scripts/install-mcp.sh"),
];
const installPath = installCandidates.find((p) => existsSync(p));
if (!installPath) {
  console.error("build-docs: install.sh not found next to docs in", installCandidates);
  process.exit(1);
}
let installSh = readFileSync(installPath, "utf8");
// Prefer the public domain in the script's own help text when served from the site.
installSh = installSh.replaceAll(
  "https://raw.githubusercontent.com/shaferllc/keel/main/scripts/install.sh",
  "https://keeljs.com/install.sh",
);
installSh = installSh.replaceAll(
  "https://raw.githubusercontent.com/shaferllc/keel/main/scripts/install-mcp.sh",
  "https://keeljs.com/install.sh",
);

// Bake llms.txt / llms-full.txt from the framework repo, with the GitHub doc
// links rewritten to this site's own /docs/* pages — an agent that lands on
// keeljs.com/llms.txt stays on keeljs.com.
const llmsRoot = join(docsDir, "..");
const readLlms = (name) => {
  const p = join(llmsRoot, name);
  if (!existsSync(p)) {
    console.error(`build-docs: ${name} not found in ${llmsRoot}`);
    process.exit(1);
  }
  return readFileSync(p, "utf8").replaceAll(
    /https:\/\/github\.com\/shaferllc\/keel\/blob\/main\/docs\/([a-z0-9-]+)\.md/g,
    "https://keeljs.com/docs/$1",
  );
};
const llmsTxt = readLlms("llms.txt");
const llmsFullTxt = readLlms("llms-full.txt");
...
/** MCP installer script body for GET /install.sh */
export const INSTALL_SH = ${JSON.stringify(installSh)};

/** llms.txt / llms-full.txt from the framework repo, doc links pointed at this site. */
export const LLMS_TXT = ${JSON.stringify(llmsTxt)};
export const LLMS_FULL_TXT = ${JSON.stringify(llmsFullTxt)};
`;
...
console.log(`build-docs: rendered ${Object.keys(pages).length} guides from ${docsDir} (v${version}); install.sh baked`);
